...
class FrameConsumer:
  def __call__(self, src_image: np.ndarray, set_exit_cond: Callable, model: ort.InferenceSession):
  ...
  def __init__(self, demo_data: dict, landmarker: FaceLandmarks, alignment: FaceAlignment):
  ...
  def _image_to_inputs(self, image: np.ndarray):
  ...
  def _model_inference(self, model: ort.InferenceSession, data_dict: dict):
  ...
  def process(self, frame: np.ndarray, model: ort.InferenceSession):
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='run demo for ITrackerPlus.')
  ...
  model_group.add_argument(
    '--onnx-file', type=str, required=True,
    help='exported onnx file for ITrackerPlus.'
  )
