...
from opengaze.model.gaze_dp import TdGazeNetReal
...
class FrameConsumer:
  def __call__(self, src_image: np.ndarray, set_exit_cond: Callable, model: TdGazeNetReal):
  ...
  def device_adapter(self):
    return self.demo_data['side_packet']['device_adapter']
  ...
  def _model_data_dict(self, face_crop: np.ndarray, face_bbox: np.ndarray):
    ...
    device = torch.tensor([self.device_adapter], dtype=torch.long)
    return dict(face=face, bbox=bbox, device=device)

  def _model_inference(self, model: TdGazeNetReal, data_dict: dict):
    data_dict = {k:v.to(self.device) for k, v in data_dict.items()}
    with torch.no_grad():
      model_outputs = model(**data_dict)
    reye_gaze, leye_gaze = [o.to('cpu') for o in model_outputs]

    output_dict = dict(reye_gaze=reye_gaze[0], leye_gaze=leye_gaze[0])
  ...
  def _point_of_gaze(self, gaze: np.ndarray, theta: float):
    # Note: 1. convert PoG from prediction space of TdGazeNet
    #       2. convert unit length from 1cm (camera) to 1mm (screen)
    #       3. the screen is in parallel to camera's XoY plane
    #
    # Point of Gaze: X-axis points rightward, Y-axis points upward
    #                Origin is at the center of the pinhole camera

    gaze_c = 1e1 * self._rotate_gaze(-10 * gaze, theta)
  ...
  def _model_post_proc(self, align_dict: dict, output_dict: dict):
    reye_gaze_c, reye_gaze_s = self._point_of_gaze(
      gaze=output_dict['reye_gaze'],
      theta=align_dict['theta'],
    )
    leye_gaze_c, leye_gaze_s = self._point_of_gaze(
      gaze=output_dict['leye_gaze'],
      theta=align_dict['theta'],
    )

    proc_dict = dict(
      reye_gaze_c=reye_gaze_c, reye_gaze_s=reye_gaze_s,
      leye_gaze_c=leye_gaze_c, leye_gaze_s=leye_gaze_s,
    )

    return proc_dict

  def process(self, frame: np.ndarray, model: TdGazeNetReal):
    ...
    data_dict = self._model_data_dict(align_dict['face_crop'], align_dict['face_bbox'])
...
def load_wrapped_model(demo_data: dict, state_dict_file: str, device: torch.device):
  model = TdGazeNetReal(**demo_data['model']).to(device=device)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='run demo for TdGazeNetReal.')
